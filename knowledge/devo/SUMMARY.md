...
│   ├── 01-core.js          (~557 lines: globals, AI/Gemini, image cache, markdown, cross-ref, Strong's modal)
│   ├── 02-data.js          (~297 lines: bible data load, IDB, scroll lock, theme, favorites/comments globals)
│   ├── 03-tts.js           (~705 lines: full TTS system — synthesis, queue, playback, word highlight)
│   ├── 04-passage.js       (~2135 lines: passage load, AI reflection/chat, summary, controls, smart reflection retry)
│   ├── 05-render-init.js   (~947 lines: comments rendering, summary, init/event wiring, push)
│   ├── 06-notes.js         (~1093 lines: notes app — sessions, detail view, standalone editor)
│   ├── 07-immersive.js     (~497 lines: immersive TTS overlay)
│   ├── 08-story.js         (~1262 lines: story modal, reflect modal w/ retry button wiring, verse peek)
│   ├── 09-soap.js          (~966 lines: SOAP screen, dashboard, list panel)
│   ├── 10-creator-canvas.js (~2200 lines: image creator, canvas mode, main toolbar, book/chapter picker, study-intro)
