...
- **Entry:** `index.html`, `js/data.js` (all content — prices, variants, colors, promos), `js/app.js`, `style.css`, `poster/` (internal promo-image studio), `register/` (QR lead capture → inbox).
...
- **Quirks:**
  ...
  - **Registration → inbox (2026-08-19): a real Google Form** (https://forms.gle/sH76wAU3CHbHziaG6) is the front door Charlie asked for — built in code by `createLeadForm()` in `sherill/register/apps-script.gs`, answers filed into the *drive-with-sherill — leads* sheet, and an **onFormSubmit trigger** emails every field to Sherill + Charlie with the customer as reply-to (Forms' own notification reaches the owner only and carries no answers). ⚠️ A trigger installed on the FORM gets `e.response`, **not** `e.namedValues` — that mistake mailed one blank lead before it was fixed.
  - **One script now serves several agents (2026-08-19).** Sherill (https://forms.gle/sH76wAU3CHbHziaG6 → *drive-with-sherill — leads*) and Victor Alvear (https://forms.gle/AoV3LhXcBBwtfy3p7 → *drive-with-victor — leads*, `vicalvear13@gmail.com`). `AGENTS` in `apps-script.gs` holds each one's inbox/mobile/form title; `createLeadForm('<key>')` builds a form + spreadsheet + trigger for a new agent. Charlie is on no distribution list — only `[TEST]` runs reach him. ⚠️ Leads are routed by **form title**, so renaming a form in the Forms UI misroutes them.
...
- **Mode wheel + modifier card (2026-09-24):** every round draws a mode — Smash (hearts), King of the Hill, Hot Potato, Tag, Coin Rush — and usually one of ten modifiers (Low Gravity, Ice, Bouncy, Turbo, Lights Out, Swap, Lava Rising, Bazooka Party, Royal Visit, Gunfight). All of it lives in `js/rounds.js`, which `sim.js` and `screen.js` both call, so the twins cannot drift. **Hearts only exist in Smash** — in the other modes a hit knocks you back (hard, for Bazooka/One Punch) instead. **Grab and throw** is the power-up button with empty hands. `?mode=hill&mod=lava` on the screen URL pins the wheel; tests pin it with `configure({ round })`. A "loot then fight" rework was tried first and rejected by Charlie — it made the Bazooka, King, Gun and Excalibur rarer, which were the parts they missed.
- **`nettest` bend p99 is seed-sensitive:** any extra `rng()` in `startRound` shifts the stream so the bench bot falls into the edge pit at t≈23 and the pre-existing pit-fall correction (~1.1 tiles) fails the 0.5 bar. Proven on the old code with four bare `rng()` calls. It is not a netcode regression; fixing the fall correction itself is the real follow-up.
- **Quirks:**
...
- **Quirks:**
  ...
  - **The VS Code twin lists recent sessions (2026-08-26).** Under the meters, the 5 newest transcripts in `~/.claude/projects/<flattened-cwd>/` — labelled with each session's first prompt — click to open a terminal on `claude --resume <id>`. Display-only: `claudeUsage.recentSessions` caps how many are *shown*, nothing is ever pruned from disk. Titles read from the first 256 KB of each `.jsonl` and cached per session id (68 files in ~4 ms); `fs.watch` on the dir, debounced 8 s.
