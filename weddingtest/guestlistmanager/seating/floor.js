...
onValue(seatingRef, (snap) => {
  ...
  tryRender();
  applyOneTimeTableMigration();
  applyMemberBasedRename();
});

// One-time renumber/cleanup requested by Charlie 2026-05-15. Safe to remove
// once it's been applied (flag stored in localStorage).
let _migrationCheckedThisSession = false;
function applyOneTimeTableMigration() {
  const FLAG = "floor:tableMigration:v3";
  if (_migrationCheckedThisSession) return;
  if (localStorage.getItem(FLAG)) return;
  if (!groups.length || !allGuests.length) return;
  ...
  // Detect the couple seat: any 2-member group containing Charlie + Karla.
  const coupleTable = groups.find((g) => {
    if (!g.memberIds || g.memberIds.length !== 2) return false;
    const names = g.memberIds
      .map((id) => allGuests.find((x) => x.id === id))
      .filter(Boolean)
      .map((x) => firstName(x.name).toLowerCase());
    return names.includes("charlie") && names.includes("karla");
  });
  const coupleNeedsRename = coupleTable && coupleTable.name !== "Couple Seat";
  const coupleNeedsCap = coupleTable && coupleTable.capacity !== 2;

  const empties = groups.filter(
    (g) =>
      g !== coupleTable &&
      (g.name === "Table 5" || g.name === "Table 8") &&
      g.memberIds.length === 0 &&
      (!g.memberMissing || g.memberMissing.length === 0)
  );
  const toRename = groups.filter(
    (g) =>
      g !== coupleTable &&
      renames[g.name] &&
      renames[g.name] !== g.name
  );
  const capacityChanges = { "Kids Table": 8 };
  const toRecap = groups.filter(
    (g) => capacityChanges[g.name] && g.capacity !== capacityChanges[g.name]
  );
  ...
  if (
    ...
    localStorage.setItem(FLAG, "noop");
    return;
  }

  const summary =
    "Apply table renumbering?\n\n" +
    (coupleTable && (coupleNeedsRename || coupleNeedsCap)
      ? `• Couple Seat: ${coupleTable.name}${
          coupleNeedsRename ? " → Couple Seat" : ""
        }${coupleNeedsCap ? `, capacity ${coupleTable.capacity} → 2` : ""}\n\n`
      : "") +
    (empties.length
      ? `• Delete empty: ${empties.map((g) => g.name).join(", ")}\n\n`
      : "") +
    (toRename.length
      ? `• Renames:\n${toRename
          .map((g) => `   ${g.name} → ${renames[g.name]}`)
          .join("\n")}\n\n`
      : "") +
    (toRecap.length
      ? `• Capacity:\n${toRecap
          .map((g) => `   ${g.name}: ${g.capacity} → ${capacityChanges[g.name]}`)
          .join("\n")}`
      : "");

  if (!confirm(summary)) return;

  const emptyIds = new Set(empties.map((g) => g.id));
  const next = groups
    .filter((g) => !emptyIds.has(g.id))
    .map((g) => {
      let out = g;
      // Couple seat: rename by ID, set capacity to 2.
      if (coupleTable && g.id === coupleTable.id) {
        out = { ...out, name: "Couple Seat", capacity: 2 };
      } else if (renames[out.name]) {
        out = { ...out, name: renames[out.name] };
      }
      if (capacityChanges[out.name]) {
        out = { ...out, capacity: capacityChanges[out.name] };
      }
      return out;
    });
...
// Rename round tables based on the FIRST occupant's first name. Charlie
// dictated this mapping on 2026-05-15 to match how he/Karla mentally label
// their tables ("Clettes' table", "Rainer's table", etc.). Tables not in the
// mapping (Couple, VIPs, Kids, and any table whose first occupant isn't
// listed) are left untouched. Safe to remove once the v1 flag is applied.
let _memberRenameCheckedThisSession = false;
...
function applyMemberBasedRename() {
  const FLAG = "floor:memberRename:v1";
  if (_memberRenameCheckedThisSession) return;
  if (localStorage.getItem(FLAG)) return;
  if (!groups.length || !allGuests.length) return;
  _memberRenameCheckedThisSession = true;

  // Key = first-member's first name (normalized, accent-stripped).
  const memberRenames = {
    clettes: "Table 3",
    charles: "Table 4",
    erika: "Table 5",
  ...
  for (const g of groups) {
    ...
    const guest = allGuests.find((x) => x.id === g.memberIds[0]);
    if (!guest) continue;
    const fn = _normalizeName(firstName(guest.name));
    const target = memberRenames[fn];
    if (target && g.name !== target) {
      proposals.push({ id: g.id, from: g.name, to: target, who: guest.name });
    }
  }
  ...
  const summary =
    "Rename tables by first occupant?\n\n" +
    proposals
      .map((p) => `  ${p.from} → ${p.to}   (first: ${p.who})`)
      .join("\n");
  if (!confirm(summary)) return;

  const byId = new Map(proposals.map((p) => [p.id, p.to]));
  for (const g of groups) {
    if (byId.has(g.id)) g.name = byId.get(g.id);
  }
  persist();
  localStorage.setItem(FLAG, new Date().toISOString());
  toast("Table names updated ✓");
  tryRender();
}
