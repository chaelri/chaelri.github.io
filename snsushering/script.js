...
const COUNTERS = [
  ...
  { id: "right", name: "Right" },
  { id: "tech", name: "Tech" },
  { id: "toddlers", name: "Toddlers Area" },
  { id: "kids", name: "Next Gen" },
  { id: "admin", name: "Admin Office" },
];
const REMINDERS = [
  "Gather Volunteers after Heart Prep",
  "Pwesto na by 4:30pm",
  "Smile and greet with joy and enthusiasm",
  "Drink water para iwas ngalay",
  "During the message pwede na pumasok",
...
function sync() {
  ...
  onValue(ref(db, `sessions/${dateKey}/assignments`), (snap) => {
    ...
    DEPLOYMENT.forEach((s) => {
      for (let i = 0; i < s.slots; i++) {
        ...
        if (val) {
          btn.className =
            "slot-btn active w-full p-4 rounded-2xl flex justify-between items-center text-[13px] font-black";
          btn.innerHTML = `<span>${val}</span> <span class="material-icons text-blue-500">check_circle</span>`;
        } else {
          btn.className =
            "slot-btn w-full p-4 rounded-2xl flex justify-between items-center text-[13px] font-bold text-slate-300";
  ...
  onValue(ref(db, `sessions/${dateKey}/counters`), (snap) => {
    ...
    COUNTERS.forEach((c) => {
      ...
      document.getElementById(`val_${c.id}`).innerText = val;
      total += val;

      // Logic: kids are toddlers and next gen. adults are everything else.
      if (c.id === "toddlers" || c.id === "kids") {
        kidsSum += val;
      } else {
        adultSum += val;
      }
...
function generateSummary() {
  ...
  COUNTERS.forEach((c) => {
    ...
    summary += `${c.name}: ${val}\n`;
    total += val;
    if (c.id === "toddlers" || c.id === "kids") kidsSum += val;
    else adultSum += val;
  });
...
window.closeDrawer = () =>
  document.getElementById("assignDrawer").classList.add("hidden");
