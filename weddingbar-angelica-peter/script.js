...
// =============================
// Couple settings (see config.js)
// =============================
const CFG = window.WEDDING_CONFIG || {};
const PARTNER_A = CFG.partnerA || "Partner 1";
const PARTNER_B = CFG.partnerB || "Partner 2";
const LOCALE = CFG.locale || "en-PH";
const CURRENCY = CFG.currency || "PHP";

// Guest "side" is stored as a neutral key so the couple can rename
// themselves in config.js at any time without rewriting saved guests.
const SIDE_LABELS = {
  partnerA: PARTNER_A,
  partnerB: PARTNER_B,
  both: "Both",
};
const sideLabel = (v) => SIDE_LABELS[v] || v || "—";

// An item tagged "Free" contributes nothing to the budget — someone gifted
// it, it came bundled, whatever. Its stored total/paid are deliberately kept
// so unticking Free restores the original numbers instead of losing them.
const isFree = (it) => !!(it && it.free);
const effTotal = (it) => (isFree(it) ? 0 : Number(it && it.total) || 0);
const effPaid = (it) => (isFree(it) ? 0 : Number(it && it.paid) || 0);

if (CFG.appTitle) document.title = `${CFG.appTitle} — ${PARTNER_A} & ${PARTNER_B}`;

// All data lives under this root so it can never collide with another
// couple's copy of this template sharing the same Firebase project.
const ROOT = CFG.dataRoot || "angelicaPeter";
const PATH = `${ROOT}/weddingCosts`;
const NEXT_PATH = `${ROOT}/weddingNextSteps`;
const GUESTS_PATH = `${ROOT}/weddingGuests`;
...
function render(items = [], sortType = "none") {
  ...
  if (sortType === "booked") {
    ...
    items.sort((a, b) => effTotal(b) - effTotal(a));
  } else if (sortType === "totalLow") {
    items.sort((a, b) => effTotal(a) - effTotal(b));
    ...
    items.sort((a, b) => effPaid(a) - effPaid(b));
  } else if (sortType === "paidHigh") {
    items.sort((a, b) => effPaid(b) - effPaid(a));
  } else if (sortType === "pctLow") {
    items.sort((a, b) => {
      const pA = isFree(a) ? 1 : a.total ? a.paid / a.total : 0;
      const pB = isFree(b) ? 1 : b.total ? b.paid / b.total : 0;
      return pA - pB;
    });
  ...
  items.forEach((it) => {
    ...
    sub.textContent = isFree(it)
      ? `Free${Number(it.total) ? ` · was ${fmt(it.total)}` : ""}`
      : it.booked || it.paid > 0
...
// Countdown + summary
// =======================================================
// Accepts either a full date ("February 14, 2027") or just a month and
// year ("February 2027"). A month-only date is deliberately counted in
// months, because counting to the day would invent a day they haven't
// chosen yet. Returns null when config.js leaves the date unset.
function parseWeddingDate(input) {
  if (!input) return null;
  const raw = String(input).trim();

  const monthOnly = /^([A-Za-z]+)\s+(\d{4})$/.exec(raw);
  if (monthOnly) {
    const d = new Date(`${monthOnly[1]} 1, ${monthOnly[2]} 00:00:00`);
    if (isNaN(d.getTime())) return null;
    return { time: d.getTime(), exact: false, label: raw };
  }

  const d = new Date(`${raw} 00:00:00`);
  if (isNaN(d.getTime())) return null;
  return { time: d.getTime(), exact: true, label: raw };
}

const WEDDING = parseWeddingDate(CFG.weddingDate);
const weddingDate = WEDDING ? WEDDING.time : null;

// Header subtitle: "Angelica & Peter · February 2027"
{
  const sub = document.getElementById("coupleSub");
  if (sub) {
    sub.textContent =
      `${PARTNER_A} & ${PARTNER_B}` +
      (WEDDING ? ` · ${WEDDING.label}` : " · date to be decided");
  }
}

// An approximate date starts on months; an exact one starts on days.
let countdownMode = WEDDING && WEDDING.exact ? "days" : "months";

function updateCountdown() {
  const numEl = document.getElementById("daysLeftNumber");
  const labelEl = document.getElementById("daysLeftLabel");
  if (!numEl || !labelEl) return;

  if (!WEDDING) {
    numEl.textContent = "—";
    labelEl.textContent = "SET YOUR DATE";
    return;
  }
  ...
  if (diff <= 0) {
    numEl.textContent = "0";
    labelEl.textContent = WEDDING.exact ? "DAYS LEFT" : "IT'S THE MONTH";
    return;
  }
  ...
  // Say "about" while the exact day is still undecided.
  if (!WEDDING.exact) label = "~ " + label;
...
function updateSummary(items = []) {
  ...
  items.forEach((it) => {
    totalPaid += effPaid(it);
    grandTotal += effTotal(it);
  });
...
function showDetails(it) {
  detailPanel.innerHTML = `
    <div style="display:flex; justify-content:space-between; align-items:center; width:100%; margin-bottom:10px;">
      <button id="backBtn" class="btn ghost"
        style="padding: 12px 18px; font-size: 14px; cursor: pointer;">
        Back
      </button>
    ...
    <div style="margin-top:12px;display:grid;grid-template-columns:1fr 1fr;gap:8px;" class="detail-grid">
      ...
      <div>
        ...
        <label class="chk" style="display:inline-flex;align-items:center;margin-left:18px;">
          <input id="detailFree" type="checkbox" ${
            it.free ? "checked" : ""
          } />
          <span class="box" aria-hidden="true"></span>
          <span class="chk-label" style="margin-left:6px;">Free</span>
        </label>
        <div class="muted" style="margin-top:6px;font-size:11px;">
          Free = counted as ₱0. Your Total below is kept, so unticking restores it.
        </div>
      </div>
  ...
  updateBtn2.onclick = async () => {
    ...
    const newFree = document.getElementById("detailFree").checked;
...
addBtn.onclick = async () => {
  ...
  if (typeof window.closeAddCostModal === "function") {
...
// ---- Timeline binder, opened in-page ----
...
if (openTimelineBtn && timelinePanel && timelineFrame) {
  openTimelineBtn.addEventListener("click", () => {
    // Load on first open so the binder costs nothing until it's wanted.
    if (!timelineFrame.getAttribute("src")) {
      timelineFrame.setAttribute("src", "timeline/");
    }
    timelinePanel.hidden = false;
    requestAnimationFrame(() => timelinePanel.classList.add("is-open"));
...
if (closeTimelineBtn) closeTimelineBtn.addEventListener("click", closeTimelinePanel);
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape" && timelinePanel && !timelinePanel.hidden) {
    closeTimelinePanel();
  }
});

// ---- Add-expense modal (same dialog on mobile and desktop) ----
const toggleBtn = document.getElementById("toggleControlsBtn");
const addCostModal = document.getElementById("addCostModal");
const addCostClose = document.getElementById("addCostClose");
let lastFocusedBeforeModal = null;

function openAddCostModal() {
  if (!addCostModal) return;
  lastFocusedBeforeModal = document.activeElement;
  addCostModal.hidden = false;
  requestAnimationFrame(() => addCostModal.classList.add("is-open"));
  document.body.style.overflow = "hidden";
...
function closeAddCostModal() {
  ...
  document.body.style.overflow = "";
  setTimeout(() => {
    addCostModal.hidden = true;
  }, 200);
  if (lastFocusedBeforeModal) lastFocusedBeforeModal.focus();
}
window.closeAddCostModal = closeAddCostModal;

if (toggleBtn) toggleBtn.addEventListener("click", openAddCostModal);
if (addCostClose) addCostClose.addEventListener("click", closeAddCostModal);

// Click the dimmed backdrop (but not the sheet) to dismiss.
if (addCostModal) {
...
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape" && addCostModal && !addCostModal.hidden) {
    closeAddCostModal();
  }
});
  ...
  sorted.forEach((it) => {
    html += `
      <tr data-id="${it.id}">
        ...
        <td>${isFree(it) ? "—" : fmt(it.paid)}</td>
        <td>${isFree(it) ? "Free" : fmt(it.total)}</td>
        <td>
...
async function uploadToFirebaseStorage(itemId, fileBlob) {
  ...
  const fileRef = sRef(storage, `${PATH}/${itemId}/${randomName}`);
...
function triggerCelebration(rowEl, anchorEl) {
  try {
    ...
    const colors = ["#9f7fb8", "#ffd56b", "#c8a2c8", "#dabfd4", "#dabfd4"];
    for (let i = 0; i < 14; i++) {
...
function loadGuests() {
  ...
  guestsUnsub = onValue(ref(db, GUESTS_PATH), (snap) => {
    ...
    if (statsBox) {
      ...
      const sideA = rawGuests.filter((g) => g.side === "partnerA").length;
      const sideB = rawGuests.filter((g) => g.side === "partnerB").length;
      ...
      statsBox.innerHTML = `
    Total: <b>${total}</b> •
    ${escapeHtml(PARTNER_A)}: ${sideA} •
    ${escapeHtml(PARTNER_B)}: ${sideB} •
    Both: ${both} •
  ...
  `;
    ...
    rawGuests.forEach((g) => {
      // Unset or unrecognised sides fall into "Both" rather than throwing.
      (groups[g.side] || groups.both).push(g);
    });
    ...
    // Render by group
    ["partnerA", "partnerB", "both"].forEach((side) => {
      if (groups[side].length === 0) return;

      // header
      const h = document.createElement("div");
      h.textContent =
        sideLabel(side).toUpperCase() + ` (${groups[side].length})`;
...
function toggleInlineGuestEditor(g, container) {
  ...
  wrap.innerHTML = `
    ...
    <select id="inlineSide-${g.id}"
      ...
      <option value="partnerA" ${
        g.side === "partnerA" ? "selected" : ""
      }>${escapeHtml(PARTNER_A)}</option>
      <option value="partnerB" ${
        g.side === "partnerB" ? "selected" : ""
      }>${escapeHtml(PARTNER_B)}</option>
...
function renderGuestChips() {
  ...
  const chips = [
    { type: "side", value: "partnerA" },
    { type: "side", value: "partnerB" },
  ...
  chips.forEach((c) => {
    ...
    // Side chips show the real names; every other chip shows its own value.
    btn.textContent = c.type === "side" ? sideLabel(c.value) : c.value;
...
// PANEL OPENERS
// =======================================================
// The header "Checklist" button opens the panel directly — no dropdown,
// and no Guests / Seating Planner entries (both removed for this build).
  ...
  bar.innerHTML = `
      <form id="guestsForm" style="width:100%; display:flex; flex-direction:column; gap:10px;">
        ...
        <select id="guestSideInput"
          ...
          color:white; border:none; font-size:16px;">
          <option value="">Side</option>
          <option value="partnerA">${escapeHtml(PARTNER_A)}</option>
          <option value="partnerB">${escapeHtml(PARTNER_B)}</option>
