// Table Numbers — one printable card per table, 1-up A4 cut-out sheet.
//
...
async function mount() {
  ...
  dlBtn.addEventListener("click", async () => {
    try {
      if (printMode) {
        const s = currentSheetIdx();
        await downloadPng(sheetSvgFor(tablesForSheet(s)), `Table Cards — sheet ${s + 1}.png`, { scale: 1 });
      } else {
        const t = tables()[previewIdx];
        if (!t) { showToast("List is empty", "err"); return; }
        await downloadPng(cardSvgFor(t), `Table Card — ${tableSlug(t)}.png`, { scale: 2 });
      }
  ...
  upBtn.addEventListener("click", async () => {
    try {
      ...
      if (printMode) {
        ...
        filename = `Table Cards — sheet ${s + 1}.png`;
      } else {
        const t = tables()[previewIdx];
        if (!t) { showToast("List is empty", "err"); return; }
        blob = await svgToPngBlob(cardSvgFor(t), { scale: 2 });
        filename = `Table Card — ${tableSlug(t)}.png`;
      }
  ...
  dlAllBtn.addEventListener("click", async () => {
    ...
    try {
      for (let i = 0; i < arr.length; i++) {
        ...
        await downloadPng(cardSvgFor(arr[i]), `Table Card — ${tableSlug(arr[i])}.png`, { scale: 2 });
  ...
  pdfAllBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      pdf.save(`Table Cards — A4 print sheets.pdf`);
  ...
  upAllBtn.addEventListener("click", async () => {
    ...
    try {
      for (let i = 0; i < arr.length; i++) {
        ...
        await uploadPngBlob(blob, `Table Card — ${tableSlug(arr[i])}.png`);
      }
