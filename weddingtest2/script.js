...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let scrollYMemory = 0;
  let isJumping = false;
  const SET_COUNT = 5; // The "Runway" multiplier
  const swiper = document.getElementById("modalSwiper");

  /**
   * 1. Initialize the Infinite Runway
   */
  function initInfiniteSwiper(images, startIndex = 0) {
    currentImagesArray = images;
    swiper.innerHTML = "";

    // Create 5 sets of cards for seamless looping
    for (let i = 0; i < SET_COUNT; i++) {
      images.forEach((src, idx) => {
        const wrapper = document.createElement("div");
        wrapper.className = "modal-snap-point";
        wrapper.setAttribute("data-index", idx);
        wrapper.innerHTML = `<div class="modal-card"><img src="${src}" draggable="false" /></div>`;
    ...
    thumbContainer.innerHTML = images
      .map(
        (src, index) => `
    <img src="${src}" class="attire-thumb ${
  ...
  `
    ...
    // Position at start index in the middle set (Set #2)
    const cardsPerSet = images.length;
    const targetIndex = 2 * cardsPerSet + startIndex;

    setTimeout(() => {
      swiper.scrollLeft = targetIndex * swiper.offsetWidth;
      setupObserver();
    }, 50);
  }

  /**
   * 2. Silent Jump Logic (The Infinity Engine)
   */
  swiper.addEventListener(
    "scroll",
    () => {
      if (isJumping || currentImagesArray.length <= 1) return;

      const scrollLeft = swiper.scrollLeft;
      const width = swiper.offsetWidth;
      const setWidth = width * currentImagesArray.length;

      // If we enter the last set, jump back to middle
      if (scrollLeft >= setWidth * 4) {
        silentJump(scrollLeft - setWidth * 2);
      }
      // If we enter the first set, jump forward to middle
  ...
  /**
   * 3. Sync Swiper to Thumbnails via Observer
   */
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && !isJumping) {
            document
              .querySelectorAll(".modal-snap-point")
            ...
            currentImgIndex = originalIndex;

            // Update Thumbnails
            const thumbs = document.querySelectorAll(".attire-thumb");
            thumbs.forEach((t, i) => {
              t.classList.toggle("active", i === originalIndex);
              if (i === originalIndex) {
                t.parentNode.scrollTo({
                  left:
                    t.offsetLeft -
                    t.parentNode.offsetWidth / 2 +
                    t.offsetWidth / 2,
                  behavior: "instant",
                });
              }
  ...
  window.updateGalleryView = (index) => {
    const cardsPerSet = currentImagesArray.length;
    const targetIndex = 2 * cardsPerSet + index; // Target middle set

    swiper.scrollTo({
      left: targetIndex * swiper.offsetWidth,
      behavior: "smooth",
    });
  };

  /**
   * 5. Open/Close Modal Handlers
   */
  document
    .querySelectorAll(".attire-album-card, .story-img")
    .forEach((card) => {
      card.addEventListener("click", () => {
        // Determine data based on card type
        const role =
          card.getAttribute("data-role") || card.getAttribute("data-title");
        const description = card.getAttribute("data-description");
        const images = attireData ? attireData[role] || [card.src] : [card.src];

        scrollYMemory =
          window.pageYOffset || document.documentElement.scrollTop;

        document.getElementById("attireModalTitle").innerText = role;
        document.getElementById("attireModalDescription").innerText =
          description;
  ...
  window.closeAttireModal = () => {
    ...
    // Force an instant jump to the previous scroll position
    window.scrollTo({
      top: scrollYMemory,
      behavior: "instant",
    });
  ...
  // Handle window resizing to keep swiper centered
  window.addEventListener("resize", () => {
    const cardsPerSet = currentImagesArray.length;
    const targetIndex = 2 * cardsPerSet + currentImgIndex;
    swiper.scrollLeft = targetIndex * swiper.offsetWidth;
  });
